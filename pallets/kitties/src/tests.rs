...
fn can_auto_breed() {
    new_test_ext().execute_with(|| {
        ...
        // check the difficulty multiplier has increased for both kitties
        assert_eq!(KittiesModule::kitty_difficulty_multiplier(0), 1);
        assert_eq!(KittiesModule::kitty_difficulty_multiplier(1), 1);
        // check new kitty has a difficulty of zero
        assert_eq!(KittiesModule::kitty_difficulty_multiplier(2), 0);
    });
}
